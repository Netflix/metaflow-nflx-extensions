...
from metaflow.plugins import DATASTORES, METADATA_PROVIDERS
...
from metaflow_extensions.netflixext.plugins.conda.conda import (
    Conda,
...
def env_spec_options(func):
    ...
    @click.option(
        ...
        default=None,
        type=click.Path(exists=True, readable=True, dir_okay=False, resolve_path=True),
        help="Get the environment definition from a pyproject.toml file (poetry format).",
    )
    @click.option(
        "--using-pathspec",
        default=None,
...
def create(
    ...
    if into_dir:
        ...
        def _write_setup():
            env_dict = MetaflowCodeContent.get_env_vars_for_packaged_metaflow(
                os.getcwd()
            )
            if not env_dict:
                return False
            with open("metaflow_setup.sh", "w", encoding="utf-8") as f:
                f.write("#!/bin/sh\n")
                f.write(
                    "# This file is automatically generated by the "
                    "`metaflow environment create` command.\n"
                )
                f.write(
                    "# It sets up the environment for running Metaflow "
                    "in this directory.\n"
                )
        ...
        if code_pkg:
            start = time.time()
            obj.echo("    Downloading code package ...", nl=False)
            MetaflowPackage.cls_extract_into(
                code_pkg._code_metadata, code_pkg._code_obj, "."
            )
            code_pkg._code_obj.seek(0)
            delta_time = int(time.time() - start)
            obj.echo(" done in %d second%s." % (delta_time, plural_marker(delta_time)))

            # We write out the environment variables needed to use this environment
            # in a metaflow_setup.sh file.
            wrote_setup = _write_setup()
    ...
    if obj.quiet:
        ...
        obj.echo(
            ...
            % (
                name,
                obj.conda.root_env_dir,
                obj.conda._bins[CONDA_DEPENDENCY_RESOLVER],
                name,
            )
        )
        if into_dir and wrote_setup:
            obj.echo(
                f"Source `{into_dir}/metaflow_setup.sh` to use the installed "
...
def resolve(
    ...
    if from_str and (req_file or yml_file or pyproject_file):
        raise click.BadOptionUsage(
            "--from/--from-pathspec",
            "Cannot specify --from/--from-pathspec with -r/-f/-p",
        )
    ...
    if pyproject_file:
        parsed_python_version = _parse_toml_file(
            pyproject_file,
            new_extras,
            new_sources,
            new_pypi_deps,
...
def _parse_toml_file(
    ...
    with open(file_name, mode="r", encoding="utf-8") as f:
        return parse_toml_value(f.read(), extra_args, sources, deps, np_deps, sys_deps)

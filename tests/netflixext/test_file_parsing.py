...
def test_parse_only_src_txt(cli_runner, mock_resolver_add_environment):
    ...
    runner, env_vars = cli_runner
    file_path = os.path.join(ENV_DIR, "only-src.txt")
...
def test_parse_only_src_yml(cli_runner, mock_resolver_add_environment):
    ...
    runner, env_vars = cli_runner
    file_path = os.path.join(ENV_DIR, "only-src.yml")
    ...
    add_environment_basic_checks(
        ...
        EnvType.MIXED,
        {
            ...
            "pypi": ["graphing", "outlier-detector==0.0.3", "nflx-python-eureq"],
            "npconda": [],
        },
        {},
        {},
...
def test_parse_cbl_tools_txt(cli_runner, mock_resolver_add_environment):
    """Test that cbl_tools.txt is correctly parsed."""
    runner, env_vars = cli_runner
    file_path = os.path.join(ENV_DIR, "cbl_tools.txt")
...
def test_parse_cbl_tools_yml(cli_runner, mock_resolver_add_environment):
    """Test that cbl_tools.yml is correctly parsed with mixed conda and pip dependencies."""
    runner, env_vars = cli_runner
    file_path = os.path.join(ENV_DIR, "cbl_tools.yml")
    ...
    add_environment_basic_checks(
        ...
        EnvType.MIXED,
        {
            ...
            "pypi": ["cbl_tools==>=2.12.6", "polars==<=0.19"],
            "npconda": [],
        },
        {},
        {},
...
def test_multiple_architectures(cli_runner, mock_resolver_add_environment):
    """Test that specifying multiple architectures results in multiple add_environment calls."""
    runner, env_vars = cli_runner
    file_path = os.path.join(ENV_DIR, "pypi-simple-no-extras.txt")
    ...
    assert mock_resolver_add_environment.add_environment.call_count == 2

    # Get all calls
    calls = mock_resolver_add_environment.add_environment.call_args_list

    # Verify different architectures were used
    archs = [call[0][0] for call in calls]
    assert "linux-64" in archs
    assert "osx-64" in archs

    # Verify both calls have the same dependencies
    for call in calls:
        args, _ = call
        deps = args[1]
        # User dep should be in pypi
...
def test_skip_metaflow_deps(cli_runner, mock_resolver_add_environment):
    ...
    # When skip_metaflow_deps is True, only user-provided deps and python should be present
    deps = args[1]
    deps.pop("sys", None)  # Remove sys deps for comparison

    # Verify the structure - should only have python and user deps
    assert "python==3.10" in deps.get("conda", [])
    assert "pandas==>=1.0.0" in deps.get("conda", [])
    assert len(deps.get("pypi", [])) == 0
    assert len(deps.get("npconda", [])) == 0
    assert len(deps.get("conda", [])) == 2

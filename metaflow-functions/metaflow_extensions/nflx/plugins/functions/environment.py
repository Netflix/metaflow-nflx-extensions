...
def run_in_path(loader_func: Callable[[], Any], root_path: str) -> Any:
    ...
    try:
        ...
        # Remove the entry this call added, rather than restoring a snapshot of
        # the whole list. A snapshot restore also drops every sys.path change
        # made by anyone else while the load was open -- another thread loading
        # concurrently loses the entry it is importing through.
        try:
